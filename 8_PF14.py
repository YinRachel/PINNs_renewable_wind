...
for file in files:
    if file.endswith('.mat'):
        ...
        all_PF14.append(mat_data['PF14'].flatten()[:,None])
        count += 1  # 增加计数器
        if count == 10:  
            break
...
# X_u_train, X_val = X[:train_size], X[train_size:]
...
# print("X_val",X_val[:,1].shape)
X_col1 = tf.reshape(X_val[:, 1], (-1, 1))
PF14_pred = nu * tf.math.sin(y_pred - X_col1)
PF14_pred = tf.abs(PF14_pred)
PF14_val_flattened = PF14_val.flatten()
PF14_pred_flattened = PF14_pred.numpy().flatten()
...
time = np.linspace(t_min, t_max, len(PF14_val_flattened))[50:]

# # 绘图
plt.figure(figsize=(12, 7))
plt.plot(time, PF14_val_flattened[50:], label="real value", linestyle='-')
plt.plot(time, PF14_pred_flattened[50:], label="predict value", linestyle='--')
